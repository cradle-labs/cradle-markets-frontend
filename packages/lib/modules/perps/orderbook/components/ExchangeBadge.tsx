...
export function ExchangeBadge({
  ...
  const content = (
    <Flex align="center" gap={1.5} className={className}>
      <ExchangeIcon exchange={exchange} className={iconClassName} />
      {showLabel && (
        <Text textTransform="capitalize">{cleanName}</Text>
      )}
      {showMarketType && (
        <Badge
  ...
  return (
    <Tooltip
      label={
        <Box fontSize="xs">
          <Text fontWeight="semibold" textTransform="capitalize">{cleanName}</Text>
      ...
      hasArrow
    >
      <Flex display="inline-flex" cursor="help">
        {content}
      </Flex>
    </Tooltip>
  );
