export const colors = {
  primary: {
    '50': '#eaf6ff',
    '100': '#daedff',
    '200': '#bcddff',
    '300': '#93c6ff',
    '400': '#68a1ff',
    '500': '#457dff',
    '600': '#2554ff',
    '700': '#2048e9',
    '800': '#183bbb',
    '900': '#1d3992',
    '950': '#112055',
  },
  gray: {
    '50': '#F7FAFC',
    '100': '#EDF2F7',
    '200': '#E2E8F0',
    '300': '#CBD5E0',
    '400': '#A0AEC0',
    '500': '#718096',
    '600': '#4A5568',
    '700': '#2D3748',
    '800': '#1A202C',
    '900': '#171923',
  },
  brown: {
    '50': '#F8F3ED',
    '100': '#EBDCCC',
    '200': '#DDC6AB',
    '300': '#D0B08A',
    '400': '#C39A6A',
    '500': '#B68449',
    '600': '#92693A',
    '700': '#6D4F2C',
    '800': '#49351D',
    '900': '#241A0F',
  },
  orange: {
    '50': '#fff7ed',
    '100': '#ffedd5',
    '200': '#fed7aa',
    '300': '#fdba74',
    '400': '#fb923c',
    '500': '#f97316',
    '600': '#ea580c',
    '700': '#c2410c',
    '800': '#9a3412',
    '900': '#7c2d12',
    '950': '#431407',
  },
  red: {
    '50': '#fef4f2',
    '100': '#fde7e3',
    '200': '#fcd4cc',
    '300': '#f9b5a8',
    '400': '#f48975',
    '500': '#ea6249',
    '600': '#d7462b',
    '700': '#b43821',
    '800': '#95321f',
    '900': '#7c2e20',
    '950': '#43150c',
  },
  purple: {
    '50': '#f5f4fe',
    '100': '#ecebfc',
    '200': '#dbdafa',
    '300': '#b3aef5',
    '400': '#9f95f0',
    '500': '#7f6ae8',
    '600': '#6c4add',
    '700': '#5c38c9',
    '800': '#4d2ea9',
    '900': '#40288a',
    '950': '#26175e',
  },
  green: {
    '50': '#eafff6',
    '100': '#cdfee7',
    '200': '#a0fad4',
    '300': '#63f2be',
    '400': '#25e2a4',
    '500': '#00d395',
    '600': '#00a474',
    '700': '#008361',
    '800': '#00674e',
    '900': '#005541',
    '950': '#003026',
  },
  // sand: 'hsla(43,23%,91%,1)',
  chartBorder: {
    light: '#E5E7EB',
    dark: '#4F5764',
  },
  gradient: {
    // dusk: 'linear(to-tr, purple.300 5%, #D7CBE7 50%, #EAA879 95%)',
    // sand: 'linear(to-t, #E6C6A0 0%, #E5D3BE 100%)',

    dawnLight:
      'linear-gradient(to right, #3F5EFB, #FC466B)',
    dawnLightAlpha15:
      'linear-gradient(to right, rgba(63, 94, 251, 0.15), rgba(252, 70, 107, 0.15))',
    dawnDark:
      'linear-gradient(to right, #3F5EFB, #FC466B)',
    dawnDarkAlpha15:
      'linear-gradient(to right, rgba(63, 94, 251, 0.15), rgba(252, 70, 107, 0.15))',
    sunsetLight: 'linear-gradient(45deg, #F06147 0%, #EA9A43 100%)',
    sunsetDark: 'linear-gradient(45deg, #F06147 0%, #EA9A43 100%)',
    sandLight: 'linear-gradient(to right, #3F5EFB, #FC466B)',
    sandDark: 'linear-gradient(to right, #3F5EFB, #FC466B)',
    special:
      'linear-gradient(266.76deg, rgba(234, 168, 121, 0.5) -20.29%, rgba(179, 174, 245, 0.5) 45.08%, rgba(234, 168, 121, 0) 110.45%)',

    goldLight: 'linear-gradient(90deg, #BC874C 0%,rgb(193, 149, 17) 50%, #BC874C 100%)',
    goldDark: 'linear-gradient(90deg, #A18775 0%, #C4BC97 50%, #958471 100%)',
  },
  aura: {
    purple: '#A855F7',
  },
  sparkles: {
    corePool: {
      from: '#936927',
      to: '#F4EAD2',
    },
    rewards: {
      from: 'yellow',
      to: 'pink',
    },
    default: {
      from: '#718096',
      to: '#A0AEC0',
    },
  },
}
